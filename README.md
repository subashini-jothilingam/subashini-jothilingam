# <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="35"> Hi, I'm Subashini

### DATA ENGINEER • SNOWFLAKE • CLOUD DATA PLATFORMS

```yaml
role: Data Engineer
location: USA
focus: Snowflake,Python,SQL,AWS,Airflow,dbt,Kafka
current_mission: Building scalable cloud-native data platforms
```
---

## 🚀 About Me

I am a Data Engineer passionate about designing modern data platforms and building scalable data pipelines.

My primary focus areas include:

* ❄️ Snowflake Data Cloud
* 🐍 Python Data Engineering
* 📊 Advanced SQL Optimization
* ☁️ AWS Data Services
* 🔄 Apache Airflow Orchestration
* 🏗️ dbt Data Transformation
* ⚡ Kafka Streaming Pipelines

I enjoy transforming raw enterprise data into reliable, analytics-ready datasets that drive business decisions.

---

<h2 align="center">🛠 Tech Stack</h2>
<p align="center">Data Engineering</p>
<p align="center">
<img src="https://img.shields.io/badge/Snowflake-29B5E8?style=for-the-badge&logo=snowflake&logoColor=white" />
<img src="https://img.shields.io/badge/dbt-FF694B?style=for-the-badge&logo=dbt" />
<img src="https://img.shields.io/badge/Airflow-017CEE?style=for-the-badge&logo=apacheairflow&logoColor=white" />
<img src="https://img.shields.io/badge/Kafka-000000?style=for-the-badge&logo=apachekafka" />
<img src="https://img.shields.io/badge/Teradata-F37421?style=for-the-badge&logoColor=white" />
</p>
<p align="center">Programming</p>
<p align="center">
<img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
<img src="https://img.shields.io/badge/SQL-336791?style=for-the-badge" />
<img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black" />
<img src="https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php&logoColor=white" />
</p>
<p align="center">Cloud & DevOps</p>
<p align="center"><img src="https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws" />
  <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white" />
</p>
<p align="center">Version Control</p>
<p align="center">
<img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" />
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github" />
</p>

---

## 🔥 Featured Projects

### ❄️ Snowflake Cortex Pipeline

AI-powered data pipeline using Snowflake Cortex and Python.

**Tech Stack**

* Snowflake Cortex
* Python
* SQL
* GitHub Actions

Repository:
https://github.com/subashini-jothilingam/snowflake-pipeline-using-cortex-code

---

## 📊 Contribution Activity

<img src="https://github-readme-streak-stats.herokuapp.com/?user=subajothi980" />

---

## 🎯 Currently Learning

* Snowflake Cortex AI
* Advanced dbt Architecture
* Data Mesh Concepts
* Generative AI for Data Engineering
* Streaming Data Platforms

---

## 🤝 Open To

✔ Data Engineer Roles

✔ Snowflake Engineer Roles

✔ Analytics Engineer Roles

✔ Cloud Data Platform Opportunities

✔ Freelance Data Projects

---

## 📫 Connect With Me

* GitHub: https://github.com/subajothi980
* LinkedIn: https://linkedin.com/in/subashini-jothilingam/

---

<p align="center">

⭐ Building Data Platforms That Scale

</p>
